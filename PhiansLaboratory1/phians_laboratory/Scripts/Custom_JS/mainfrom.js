...
function init_operate_click() {
    ...
    $('#Sign_out').click(function () {
        ...
        $.ajax({
            type: 'POST',
            url: "/mainform/loginout",
          
            success: function (data) {
                if (data == "T") {
                    window.location.href = "/login/Index";
                    ////消息类型
                    //window.parent.document.getElementById('message_type').value = "123456";
                    //// 发送消息
                    //window.parent.document.getElementById('test_message').click();
                } else {
...
function init_message_Hub() {
        ...
        $.connection.hub.start().done(function () {
            chat.server.register($.cookie('SignalRID'));
            $('#send_message').click(function () {
               
                chat.server.client_message($.cookie('SignalRID'),"测试");
            });
