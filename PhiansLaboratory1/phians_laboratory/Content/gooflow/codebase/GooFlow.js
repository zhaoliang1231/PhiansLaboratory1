...
//初始化区域图的对象
	...
	this.$nodeDom={};
	this.$areaDom={};
	this.$max=property.initNum||1;//计算默认ID值的起始SEQUENCE sequence
	...
	if(this.$editable){
	  ...
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		Y=ev.y-t.top+this.parentNode.scrollTop
		  //选中项目名字
		  //选中项目名字
		This.addNode(new Date().getTime(),{name:"node_"+This.$max,left:X,top:Y,type:This.$nowType,sort:This.$max});
		This.$max++;
	  });
	  //划线或改线时用的绑定
...
GooFlow.prototype={
    ...
    //设置结点/连线/分组区域的文字信息
	setNameAndTaskid: function (id, name, type,Taskid) {
	    var oldName;
	    if (type == "node") {//如果是结点
	        if (!this.$nodeData[id]) return;
	        ...
	        oldName = this.$nodeData[id].name;
	        this.$nodeData[id].name = name;
	        this.$nodeData[id].taskid = Taskid;
	        ...
	        oldName = this.$lineData[id].name;
	        this.$lineData[id].name = name;
	        this.$lineData[id].taskid = Taskid;
	        if (GooFlow.prototype.useSVG != "") {
	            this.$lineDom[id].childNodes[2].textContent = name;
	        }
	        else {
	...
	exportData2:function(){
		...
		for(var k1 in this.$nodeData){
			...
			//ret.nodes[k1]=JSON.parse(JSON.stringify(this.$nodeData[k1]));
			node_= node_+JSON.stringify(this.$nodeData[k1])+",";
			
        }
		//去除最后一个逗号
		node_=(node_.substring(node_.length-1)==',')?node_.substring(0,node_.length-1):node_;
		//ret.nodes=JSON.parse("\\u007B"+lins_+"}");
          node_="["+node_+"]"
	     ret.nodes=JSON.parse(node_);
	     for (var k2 in this.$lineData) {
             
	         console.log(k2);
			line_= line_+JSON.stringify(this.$lineData[k2])+",";  
        // console.log(line_);			
		}
        //去除最后一个逗号
		line_=(line_.substring(line_.length-1)==',')?line_.substring(0,line_.length-1):line_;
		//ret.nodes=JSON.parse("\\u007B"+lins_+"}");
          line_="["+line_+"]"
	    ret.lines=JSON.parse(line_);
...
///////////以下为有关画线的方法
	...
	calcStartEnd:function(n1,n2,scale){
		...
		if (n1.type == "start round mix" || n2.type == "start round mix") {
		    x12 = n1.left * scale + n1.width * scale ;
		    x22 = n2.left * scale + n2.width * scale 
		}
		else {
		    x12 = n1.left * scale + n1.width * scale + 51;
		    x22=n2.left*scale +n2.width*scale+51 

		}
		var x11=n1.left*scale , x21=n2.left*scale ;
		//结点2在结点1左边
		if(x11>=x22){
			X_1=x11;X_2=x22;
		}
	...
	addLineDom:function(id,lineData){
		...
		if(GooFlow.prototype.useSVG==""){
			...
			if(lineData.type!="sl"){
				...
				((res.end[0]-res.start[0])*(res.end[0]>res.start[0]? 1:-1)-this.$lineDom[id].childNodes[1].offsetWidth)/2;
	...
	addLine:function(id,json){
		...
		this.$lineData[id].sort=json.sort;
