...
$(function () {
    ...
    Test_parameters_show_initDataGrid();

    /*性能参数*/
    Performance_parameter_load();
    Performance_parameter_show_initDatagrid();
    ...
    $("#RangeFlag").combobox({
        data: [
            ...
            if (range_text == "A＜X" || range_text == "A≦X") {
                ...
                $("#RequiredValue1").textbox({ required: true });
                $("#RequiredValue2").textbox('setText', '');
            }
            else {
                $(".hide_div").css("display", "block");
                $("#RequiredValue1").textbox({ required: true });
                $("#RequiredValue2").textbox({ required: true });
            };
            if (range_text == "X＜B" || range_text == "X≦B") {
                //要求值1隐藏
                $(".hide_div1").css("display", "none");
                $("#RequiredValue2").textbox({ required: true });
                $("#RequiredValue1").textbox('setText', '');
            }
            else {
...
function load_tree() {
    $('#Laboratory_tree').tree({
        ...
        onLoadSuccess: function () {
            ...
            if (tree_select_id != "") {
                var node = $('#Laboratory_tree').tree('find', tree_select_id);//找到id为”tt“这个树的节点id为”1“的对象
                $('#Laboratory_tree').tree('select', node.target);//设置选中该节点
        ...
        onSelect: function () {
            ...
            tree_select_id = $("#Laboratory_tree").tree('getSelected').id;
    ...
    $('#tree_add_next1').unbind('click').bind('click', function () {
        ...
        if (node) {
            ...
            case "True":
                ...
                break;
            case "False":
                $.messager.alert('Tips', 'Cannot add next node！');
                break;
                defalut: break;
            }
        }
    ...
    //编辑项目
    $('#editBasicInfo').unbind("click").bind("click", function () {
        var node = $('#Laboratory_tree').tree('getSelected');
        if (node) {
            //node.text = '修改';  //-->txt-->DB
            editBasicInfo(node);
    ...
    //copy
    $('#tree_copy').unbind("click").bind("click", function () {
        treeCopy();
    });
    //选择模板
...
function chooseTemplate() {
    ...
    if (node) {
        ...
        if (node.TemplateID_id) {
            $('#TemplateCombobox').combobox("setValue", node.TemplateID_id);
        }
            ...
            buttons: [
                {
                    ...
                    handler: function () {
                        var TemplateComboboxVal = $('#TemplateCombobox').combobox("getValue");
                        if (TemplateComboboxVal) {
                            $("#ShowOffice").prop("href", WordUrlSpit[0] + '?id=' + TemplateComboboxVal + '&Operation_Type=6&pageName=FileManagement' + WordUrlSpit[1]);
            ...
            buttons: [
                {
                    ...
                    iconCls: 'icon-cancel',
                    handler: function () {
                        $('#Copy_Review_dialog').dialog('close');
                    }
                }
            ]
        });
        copyTree();//加载拷贝模板中树信息
        TestDatagrid();//加载拷贝子项目
        MainTestDatagrid();//加载拷贝主项目
        //formReload();
    } else {
        $.messager.alert('Tips', 'Please select the tree to be operated！');
    };
...
/*
 *functionName:加载列表相同项目 TestDatagrid
 *function:加载列表相同项目projectID一样的任务列表
 *Param search，key
 *author:程媛
 *date:2018/7/23
*/
function TestDatagrid() {
    //获取url链接传参
    var MTRNO = unescape(getUrlParam('MTRNO'));
    var CurrentMTRflag = 0;
    if (CurrentMTR.checked == true) {
    ...
    $('#TestDatagrid').datagrid({
        ...
        type: 'POST',
        dataType: 'json',
        url: "/ScheduleManagement/GetSubTaskList", //接收一般处理程序返回来的json数据  
        queryParams: {
            search: $("#KeyValue").textbox("getValue"),
            key: $("#KeyValue1").textbox("getText"),
            MTRNO: MTRNO,
            CurrentMTRflag: CurrentMTRflag
        },
        columns: [[
      ...
      // { field: 'MainNum', title: 'MainNum' },
       ...
       { field: 'Conculsion', title: 'Conculsion' },
        ...
        onLoadSuccess: function (data) {
            $('#TestDatagrid').datagrid('selectRow', line);
        },
        onSelect: function (index, row) {
            $('#copyContent').form('load', row);//回显选中行的数据
        },
        sortOrder: 'asc',
        toolbar: "#TestDatagrid_toolbar"
    });
    //搜索下拉框
    $('#KeyValue').combobox({
        data: [
    ...
    //搜索
    $('#SearchCopy').unbind('click').bind('click', function () {
        SearchCopy();
    });
    //拷贝子项目
    ...
    //是否显示当前MTR项目的任务
    $('#CurrentMTR').unbind('click').bind('click', function () {
        SearchCopy();
    });
};
/*
 *functionName:SearchCopy
 *function:搜索
 *Param search,key
 *author:程媛
 *date:2018/7/23
...
function SearchCopy() {
    //获取url链接传参
    var MTRNO = unescape(getUrlParam('MTRNO'));
    var search = $("#KeyValue").combobox('getValue');//获取下拉框的值
    var key = $("#KeyValue1").textbox('getText');//获取搜索输入框的文本值
    var CurrentMTRflag = 0;
    if (CurrentMTR.checked == true) {
...
function copy() {
    //获取url链接传参
    var mtrno = unescape(getUrlParam('MTRNO'));
    var getRow = $('#TestDatagrid').datagrid('getSelected');
    var nodes = $('#copyTree').tree('getSelected');
    if (getRow) {
        if (nodes) {
            var parentID = "";
            ...
            $.messager.confirm('Tips', 'Are you sure want to copy the project？', function (r) {
                if (r) {
                    $.ajax({
                        url: "/ScheduleManagement/CopyProject",
                        type: 'POST',
                        data: {
                            TreeTaskId:nodes.id,
                            TaskId: getRow.TaskId,
                            ParentID: parentID,
                            Isparent: nodes.Isparent,
                            MTRNO: mtrno,
                            ProjectName: $("#ProjectName1").textbox("getText"),
            ...
            $.messager.alert('Tips', 'Please select the tree to operate！');
        }
    } else {
...
function MainTestDatagrid() {
    //获取url链接传参
    var MTRNO = unescape(getUrlParam('MTRNO'));
    var MCurrentMTRflag = 0;
    if (MCurrentMTR.checked == true) {

        MCurrentMTRflag = 1;

    }

    $('#MainTestDatagrid').datagrid({
        nowrap: false,
        striped: true,
        rownumbers: true,
        border: false,
        ...
        type: 'POST',
        dataType: 'json',
        url: "/ScheduleManagement/GetMainTaskList", //接收一般处理程序返回来的json数据  
        queryParams: {
            search: $("#MainKeyValue").textbox("getValue"),
            key: $("#MainKeyValue1").textbox("getText"),
            MTRNO: MTRNO,
            MCurrentMTRflag: MCurrentMTRflag
      ...
      // { field: 'MainNum', title: 'MainNum' },
       ...
       { field: 'Conculsion', title: 'Conculsion' },
        ...
        sortOrder: 'asc',
        toolbar: "#MainTestDatagrid_toolbar"
    });
    //搜索下拉框
    $('#MainKeyValue').combobox({
        data: [
            { 'value': 'MTRNO', 'text': 'MTRNO' },
           { 'value': 'ProjectName', 'text': 'ProjectName' }
        ]
    ...
    //搜索
    $('#MainSearchCopy').unbind('click').bind('click', function () {
        MainSearchCopy();
    });
    //拷贝主项目
    $("#MainCopy").unbind("click").bind("click", function () {
        MainCopy();
    });
    //是否显示当前MTR项目的任务
    $('#MCurrentMTR').unbind('click').bind('click', function () {
        MainSearchCopy();
    });

};
/*
 *functionName:MainSearchCopy
 *function:搜索
 *Param search,key
 *author:程媛
 *date:2018/7/23
*/
function MainSearchCopy() {
    //获取url链接传参
    var MTRNO = unescape(getUrlParam('MTRNO'));
    var search = $("#MainKeyValue").combobox('getValue');//获取下拉框的值
    var key = $("#MainKeyValue1").textbox('getText');//获取搜索输入框的文本值
    var MCurrentMTRflag = 0;
    if (MCurrentMTR.checked == true) {
...
function MainCopy() {
    var mtrno = unescape(getUrlParam('MTRNO'));
    var getRow = $('#MainTestDatagrid').datagrid('getSelected');
    var nodes = $('#copyTree').tree('getSelected');
  
    if (getRow) {
        if (nodes) {
            if (nodes.id != "a447d88b-c84a-4746-9d78-2842011def77" && nodes.Isparent == "True") {
                $.messager.confirm('Tips',
                    'Are you sure want to copy the project？',
                    function(r) {
                        if (r) {
                            $.ajax({
                                url: "/ScheduleManagement/CopyMainProject",
                                type: 'POST',
                                data: {
                                    TaskId: getRow.TaskId,
                                    SourceTaskId: nodes.id,
                                    MTRNO: mtrno,
                                    MainProjectName: $("#ProjectName1").textbox("getText"),
                                    ProjectName: getRow.ProjectName
                                },
                                success: function(data) {
                                    var result = $.parseJSON(data);
                                    if (result.Success == true) {
                ...
                $.messager.alert('Tips', 'Please select the main project！');
...
//修改基础信息  程媛 2018-10-10
function editBasicInfo(node) {
    $('#editBasicInfoDialog').dialog({
        ...
        buttons: [{
            ...
            iconCls: 'icon-save',
            handler: function () {
                $('#editBasicInfoDialog').form('submit', {
                    url: "/ScheduleManagement/EditProjectBaseInfo",//接收一般处理程序返回来的json数据     
                    onSubmit: function (param) {
                        ...
                        if (result.Success == true) {
                            ...
                            if (node1) {
                                $('#Laboratory_tree').tree('update', {
                                    ...
                                    target: node.target,
                                    text: $('#NewProjectName').textbox('getText')
                                });
                            }
            ...
            iconCls: 'icon-cancel',
            handler: function () {
                $('#editBasicInfoDialog').dialog('close');
            }
        }]
    });
    // form数据回显
    $.ajax({
        url: "/ScheduleManagement/GetTaskInfo",
        type: 'POST',
        data: {
            TaskId: node.id
        },
        success: function (data) {
            if (data) {
                var result = $.parseJSON(data);
                if (result.Success == true) {
                    $("#NewProjectName").textbox("setValue", result.Data.ProjectName);
                    $("#NewTotal").textbox("setValue", result.Data.Total);
                }
...
function Performance_parameter_load() {
    ...
    //显示（任务Task）下所有的性能参数（performance parameter）
    $("#Performance_parameters_info_showDialog").unbind("click").bind("click", function () {
        Performance_parameters_info_showDialog();
    });
...
function Performance_parameter_show_initDatagrid() {
    //初始化datagrid
    $("#performance_parameter_show_datagrid").datagrid({
        nowrap: false,
        striped: true,
        singleSelect: true,
        fit: true,
        ...
        toolbar: Performance_parameters_show_toolbar
    });
    //删除
    $("#Performance_parameters_show_delete").unbind("click").bind("click", Performance_parameters_show_delete);
    //编辑
    $("#Performance_parameters_show_edit").unbind("click").bind("click", Performance_parameters_show_edit);
}
//添加性能参数dialog
...
function Performance_parameters_info_edit() {
    ...
    if (selectRow_Performance_parameter_datagrid) {
        ...
        //数据范围回显
        var RangeFlagCombobox = selectRow_Performance_parameter_datagrid.RangeFlag;
        if (RangeFlagCombobox === null) {
            $("#RangeFlag").combobox("setValue", 'A≤X≤B');
            $("#RequiredValue1").textbox('textbox').validatebox('options').required = true;
            $("#RequiredValue2").textbox('textbox').validatebox('options').required = true;
        } else {
...
function Jsrule_Edit_Submit(node_on1, selectRow_Performance_parameter_datagrid) {
    //RangeFlag的值
    var RangeFlag_ = $("#RangeFlag").combobox("getValue");
    //判断AutoFlag是否自动
    var AutoFlag_ = $("#AutoFlag").val();
    ...
    var selectRow_contation_datagrid = $('#contation_datagrid').datagrid('getSelected');//获取条件ID
    var selectRow_Performance_parameter_datagrid1 = $('#Performance_parameters_info_datagrid').datagrid('getSelected');
    $('#add_Performance_detail').form('submit', {
        url: "/ScheduleManagement/EditProjParameterValue",//接收一般处理程序返回来的json数据     
        onSubmit: function (param) {
            param.GroupID = selectRow_contation_datagrid.ID;
            param.ID = selectRow_Performance_parameter_datagrid1.ID;
            param.TaskId = node_on1.id;

            param.Parameter = selectRow_Performance_parameter_datagrid1.Parameter;
            param.ProjectName = node_on1.text;
            param.SpecificationsTOP = RequiredValue2_;
            param.SpecificationsLower = RequiredValue1_;
            param.RangeFlag = RangeFlag_;
...
//显示（任务Task）下所有的性能参数（performance parameter）
function Performance_parameters_info_showDialog() {
    var condition = $('#contation_datagrid').datagrid('getSelected');
    var task = $('#Laboratory_tree').tree('getSelected');

    if (task) {

        $("#performance_parameter_show_dialog").dialog({
            title: 'show',
            width: 1000,
            height: 500,
            onClose: function () {
...
//SHOW-删除
function Performance_parameters_show_delete() {
    var task = $('#Laboratory_tree').tree('getSelected');
    var select = $('#performance_parameter_show_datagrid').datagrid('getSelected');
    if (task) {
        if (select) {
            $.messager.confirm('confirm', 'confirm delete?', function (r) {
                if (r) {
                    $.ajax({
                        ...
                        success: function (data) {
                            var result = $.parseJSON(data);
                            if (result.Success == true) {
                                $('#performance_parameter_show_datagrid').datagrid('reload');
                                $.messager.alert('tips', result.Message);
                            } else {
                                $.messager.alert('tips', result.Message);
                            }
...
//SHOW-编辑
function Performance_parameters_show_edit() {
    var task = $('#Laboratory_tree').tree('getSelected');
    var select = $('#performance_parameter_show_datagrid').datagrid('getSelected');
    if (select) {
        //ParameterType combobox
        ...
        $('#add_Performance_parameter_add').dialog({
            ...
            buttons: [{
                ...
                handler: function () {
                    //性能参数添加判断+提交 node_on1  selectRow_Performance_parameter_datagrid传参
                    Jsrule_Edit_Submit(task, select);
...
function Test_parameters_load() {
    ...
    //显示详情
    $("#Test_parameters_info_showDialog").unbind("click").bind("click", function () {
        Test_parameters_info_showDialog(); //显示详情
    });
}

//测试参数（SHOW）初始化化加载
function Test_parameters_show_initDataGrid() {
    //初始化datagrid
    $("#testcondition_show_datagrid").datagrid({
        nowrap: false,
        striped: true,
        singleSelect: true,
        fit: true,
        ...
        toolbar: Test_parameters_show_toolbar
    });
    //删除
    $("#Test_parameters_show_delete").unbind("click").bind("click", Test_parameters_show_delete);
    //编辑
    $("#Test_parameters_show_edit").unbind("click").bind("click", Test_parameters_show_edit);
                        ...
                        handler: function () {
                            $('#add_Test_detail').form('submit', {
                                ...
                                onSubmit: function (param) {
                                    ...
                                    //设置Commonflag的表单值
                                    if ($("#Commonflag1_").prop("checked") === true) {
                                        $("#Commonflag_").val("true");
                                    } else {
                                        $("#Commonflag_").val("false");
                                    }
...
//弹框显示详情
function Test_parameters_info_showDialog() {
    var condition = $('#contation_datagrid').datagrid('getSelected');
    var task = $('#Laboratory_tree').tree('getSelected');

    if (task) {

        $("#testcondition_show_dialog").dialog({
            title: 'show',
            width: 800,
            height: 500,
            onClose: function () {
...
//show-删除参数
function Test_parameters_show_delete() {
    var task = $('#Laboratory_tree').tree('getSelected');
    var select = $('#testcondition_show_datagrid').datagrid('getSelected');
    if (task) {
        if (select) {
            $.messager.confirm('confirm', 'confirm delete?', function (r) {
                if (r) {
                    $.ajax({
                        ...
                        success: function (data) {
                            var result = $.parseJSON(data);
                            if (result.Success == true) {
                                $("#testcondition_show_datagrid").datagrid("reload"); //刷新
                                $.messager.alert('tips', result.Message);
                            } else {
                                $.messager.alert('tips', result.Message);
                            }
...
//show-编辑参数
function Test_parameters_show_edit() {
    var task = $('#Laboratory_tree').tree('getSelected');
    var select = $('#testcondition_show_datagrid').datagrid('getSelected');
    if (select) {
                ...
                handler: function () {
                    $('#add_Test_detail').form('submit', {
                        ...
                        onSubmit: function (param) {
                            ...
                            param.ID = select.ID;
                            param.Parameter = select.Parameter;
                            param.ProjectName = task.text;
                        },
                        success: function (data) {
                            var result = $.parseJSON(data);
                            if (result.Success == true) {
