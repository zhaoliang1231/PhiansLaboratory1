...
function File_type() {
    $('#File_type').tree({
        url: "/ManageQuality/LoadPageTree?id=884fda8e-bad2-4ccc-a8b5-08caabe6e33a&Parent_id=8C20E217-5DC9-4B11-83BC-0325B47F7807",
        method: 'post',
        required: true,
        //title: '部门',
        fit: true,
        ...
        onBeforeExpand: function (node, param) {
            $('#File_type').tree('options').url = "/ManageQuality/LoadPageTree?id=884fda8e-bad2-4ccc-a8b5-08caabe6e33a&Parent_id=" + node.id;
        },
        onSelect: function () {
            treeDatagrid();
        }
...
function Environmental_managementread() {
    ...
    if (selectRow) {
        $("#ShowOffice").prop("href", WordUrlSpit[0] + '?id=' + selectRow.id + '&Operation_Type=2&pageName=FileManagement' + WordUrlSpit[1]);
