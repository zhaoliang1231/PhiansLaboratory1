...
function File_type() {
    $('#File_type').tree({
        url: "/ManageQuality/LoadPageTree?id=04a8c7e6-0f36-4e0a-8087-4241876125d0&Parent_id=8c20e217-5dc9-4b11-83bc-0325b47f7807",
        method: 'post',
        required: true,
        //title: '部门',
        fit: true,
        ...
        onBeforeExpand: function (node, param) {
            $('#File_type').tree('options').url = "/ManageQuality/LoadPageTree?id=04a8c7e6-0f36-4e0a-8087-4241876125d0&Parent_id=" + node.id;
        },
        onSelect: function () {
            treeDatagrid();
        }
...
function search_file() {
    var file_type = $('#File_type').tree('getSelected');
...
function Environmental_management_addfile() {
        ...
        $('#add_file').form('submit', {
            ...
            success: function (data) {
                ...
                if (result.Success == true) {
                    ...
                    $('#add_file').dialog('close');//关闭添加文件弹窗
    ...
    $('#FileType').textbox('setValue', node.text);//重新赋值文件类型文本框的值
};
...
function Environmental_management_alterfile() {
    ...
    var node = $('#File_type').tree('getSelected');//获取选中的树节点
    if (selectRow) {
        line = $('#Environmental_management').datagrid("getRowIndex", selectRow);
        $('#edit_file').dialog({
            width: 630,
            height: 350,
            modal: true,
...
function Environmental_managementread() {
    ...
    if (selectRow) {
        $("#ShowOffice").prop("href", WordUrlSpit[0] + '?id=' + selectRow.id + '&Operation_Type=4&pageName=FileManagement' + WordUrlSpit[1]);
...
function treeDatagrid() {
    ...
    $('#Environmental_management').datagrid({
        ...
        url: "/ManageQuality/GetFileManagement",//接收一般处理程序返回来的json数据  
        columns: [[
            { field: 'FileNum', title: 'File Num', sortable: 'true', width: 150 },
            ...
            { field: 'FileNewName', title: 'File New Name', width: 150, hidden: true },
            {
            ...
            {
                field: 'state', title: 'File state', width: 120, formatter: function (value, row, index) {
                    switch (row.state) {
                        case 0: value = "有效"; break;
                        case 1: value = "无效"; break;
                        case 2: value = "作废"; break;
                    }
                    return value;
                }
            },
        ...
        onLoadSuccess: function (data) {
            $('#Environmental_management').datagrid('selectRow', line);//默认选中第一行
        },
        rowStyler: function (index, row) {
            switch (row.state) {
                case 2: return 'color:red;'; break;
            }
        },
        sortName: 'FileNum',
        sortOrder: 'asc',
