...
$(function () {
    ...
    $('#read_search_info').unbind('click').bind('click', function () {
        var selectRow = $("#report_review_task").datagrid("getSelected");
        $('#read_table').datagrid({
            queryParams: {
...
function returnHistoryReport() {
    var select = $('#report_review_task').datagrid("getSelected");
    if (select) {
        $.messager.confirm('提示', '您确认要退回审核吗', function (r) {
            if (r) {
                $.ajax({
                    url: "/ReportManagement/TakeBackReviewReport",
                    type: 'POST',
                    data: {
                        id: select.id,
                ...
                $('#report_review_task').datagrid(
                    {
                        ...
                        type: 'POST',
                        dataType: "json",
                        url: "/ReportManagement/LoadReportReviewList",//接收一般处理程序返回来的json数据        
                        columns: [[
                            ...
                            for (var i = 0; i < hiddenJson.length; i++) {
                                try {
                                    if (hiddenJson[i].hidden) {
                                        $('#report_review_task').datagrid('hideColumn', hiddenJson[i].fieldname);
                                    } else {
                                        $('#report_review_task').datagrid('showColumn', hiddenJson[i].fieldname);
                                    }
                                } catch (e) {
                        ...
                        onLoadSuccess: function (data) {
                            //默认选择行
                            $('#report_review_task').datagrid('selectRow', line);
                            var selectRow = $("#report_review_task").datagrid("getSelected");
                            if (!selectRow) {
                                $('#report_review_task').datagrid('selectRow', line);
                            }
...
//    var selectRow = $("#report_review_task").datagrid("getSelected");
//    if (selectRow) {
//        line = $('#report_review_task').datagrid("getRowIndex", selectRow);
...
//            var selectRow_report = $("#report_review_task").datagrid("getSelected");
...
//                url: "LosslessReport_Review.ashx",//接收一般处理程序返回来的json数据     
...
//                    if (r) {
//                        $.ajax({
//                            url: "LosslessReport_Review.ashx?&cmd=del_accessory",
//                            dataType: "text",
//                            type: 'POST',
//                            data: {
...
function submit_report() {
    ...
    if (selected_report) {
        //var rowss = $("#report_review_task").datagrid("getSelections");
        //var id = [];
        //var report_num = [];
        //var report_url = [];
        //var tm_id = [];
        ...
        //    tm_id.push(rowss[i].tm_id);
        //}
        //var ids = id.join(",");
        //var report_nums = report_num.join(",");
        //var report_urls = report_url.join(",");
        //var tm_ids = tm_id.join(",");


        $('#level_Audit').combobox({
            value: 0,
            data: [
                  { 'value': '0', 'text': 'Ⅱ' },
                  { 'value': '1', 'text': 'Ⅲ' }
        ...
        $('#level_date').datetimebox({
            disabled: true,
            required: true,
            value: loaddatetime(new Date())
        });
        //判断是二级还是三级审核
        var level_ = 0;
        $.ajax({
            url: "/ReportManagement/LoadLevel",
            type: 'POST',
            data: {
                ...
                if (result.Message == '2') {
                    ...
                    $('#submit_dialog').dialog({
                        ...
                        buttons: [{
                            ...
                            iconCls: 'icon-ok',
                            handler: function () {
                                $.messager.confirm('完成提示', '您确认要完成报告吗', function (r) {
                                    if (r) {
                                        SubmitIssue(selected_report, selected_report.id, selected_report.report_num, selected_report.report_url, selected_report.tm_id, level_);
...
//提交报告签发
function SubmitIssue(select, ids, report_nums, report_urls, tm_ids, level_) {


    $('#submit_dialog').form('submit', {
        url: "/ReportManagement/SubmitReviewReport",//接收一般处理程序返回来的json数据     
        onSubmit: function (param) {
            //param.cmd = 'SubmitReviewReport';
            param.ids = ids;
            //param.Inspection_personnel = select.Inspection_personnel;
            param.level_date = $("#level_date").datetimebox("getValue");
            param.report_nums = report_nums;
            param.report_urls = report_urls;
            param.tm_ids = tm_ids;
            param.level_Audit = $("#level_Audit").combobox("getText");
        },
        success: function (data) {
            var result = $.parseJSON(data);
            if (result.Success == true) {
                $('#submit_dialog').dialog('close');
                if (level_ == 1) { $.messager.alert('提示', result.Message); }
                else {
...
function write_method_equipment1() {
    ...
    if (selectRow) {
        ...
        $('#Standard_verification1').datagrid({
            ...
            onLoadSuccess: function (data) {
                $('#Standard_verification1').datagrid('selectRow', line);
            }
        });
    } else {
        $.messager.alert('提示', '请选择你要操作得行');
...
function write_method_equipment() {
    ...
    if (selectRow) {
            ...
            buttons: [{
                ...
                handler: function () {
                    ...
                    $('#Back_report_dialog').form('submit', {
                        url: "/ReportManagement/BackReviewReport",//接收一般处理程序返回来的json数据     
                        onSubmit: function (param) {
                            //param.cmd = 'BackReviewReport';
        ...
        $('#Standard_Authorized').datagrid({
            ...
            columns: [[
                { field: 'Project_name', width: 260, title: '退回信息' }
            ]],
...
function management_all() {
    ...
    $('#group').combobox({
        url: "/ReportManagement/load_professional_department",
        valueField: 'id',
        textField: 'Department_name',
        onSelect: function () {
            //var User_department = $('#group').combobox('getValue');
            //$('#Issue_personnel').combobox({
            //    url: "LosslessReport_Review.ashx?&cmd=load_responsible_people",
