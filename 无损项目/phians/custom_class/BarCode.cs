...
namespace phians.custom_class
{
    ...
    public class BarCode
    {
        ...
        /// <returns>生成的条形码的url地址数组</returns>
        public  List<string> GetImageUrl(string barCodes)
        {
            if (!Directory.Exists(System.AppDomain.CurrentDomain.BaseDirectory + "temp\\"))//如果不存在就创建file文件夹
            {
                Directory.CreateDirectory(System.AppDomain.CurrentDomain.BaseDirectory + "temp\\");
            }
            else
            {
                //在创建新的条形码图片前清理上一次创建的图片，防止垃圾堆积
                DirectoryInfo theFolder = new DirectoryInfo(System.AppDomain.CurrentDomain.BaseDirectory + "temp\\");
            ...
            System.Drawing.Bitmap imgTemp = null;
            for (int i = 0; i < text.Length; i++)
            {
                try
                {
                    imgTemp = _Code.GetCodeImage(text[i], Code128.Encode.Code128B);
                }
                catch (Exception e)
                {
                    urls.Add("");
                ...
                name = "BarCode" + i + ".bmp";
                url = System.AppDomain.CurrentDomain.BaseDirectory + "temp\\" + name;
                imgTemp.Save(url, System.Drawing.Imaging.ImageFormat.Bmp);
                urls.Add(@"\temp\" + name);
            }
