    ...
    $('#report_review_task').datagrid(
       {
           ...
           type: 'POST',
           dataType: "json",
           url: "LosslessReport_EditApply_Audit.aspx?&cmd=load_list",//接收一般处理程序返回来的json数据        
           columns: [[
    ...
    if (selected_report) {
        ...
        $("#save2").unbind('click').bind('click', function () {
            $('#Back_report_dialog').form('submit', {
                url: "LosslessReport_EditApply_Audit.aspx",//接收一般处理程序返回来的json数据     
...
function submit_report() {
    var selected_report = $("#report_review_task").datagrid("getSelected");
    if (selected_report) {
        $.messager.confirm('提交提示', '您确认要提交选中任务吗', function (r) {
            if (!r) {
                return;
            }
            $.ajax({
                url: "LosslessReport_EditApply_Audit.aspx?&cmd=audit",
                type: 'POST',
                data: {
                    //reports_num: select_report_ed.reports_num,
                    id: selected_report.id,
                    report_id : selected_report.report_id,
                    contract_num: selected_report.contract_num,
