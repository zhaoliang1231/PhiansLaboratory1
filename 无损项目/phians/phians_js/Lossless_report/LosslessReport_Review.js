                ...
                $('#report_review_task').datagrid(
                    {
                        ...
                        type: 'POST',
                        dataType: "json",
                        url: "LosslessReport_Review.ashx?&cmd=load_list",//接收一般处理程序返回来的json数据        
                        columns: [[
    ...
    if (selected_report) {
        $.ajax({
            ...
            success: function (data) {
                ...
                new_url = strs[0];
                $("#read_doc").prop("href", new_url + "___hxw&report_url=" + data + "&report_id=" + selected_report.id);
...
function submit_report() {
    ...
    if (selected_report) {
        ...
        var level_ = 0;
        $.ajax({
            url: "LosslessReport_Review.ashx?&cmd=loadlevel",
            type: 'POST',
            data: {
        ...
        //$.ajax({
        //    url: "LosslessReport_Review.ashx?&cmd=submit_review_Report",
        //    type: 'POST',
        //    data: {
...
function SubmitIssue(select, ids, report_nums, report_urls, tm_ids, level_) {
    ...
    $('#submit_dialog').form('submit', {
        url: "LosslessReport_Review.ashx",//接收一般处理程序返回来的json数据     
        onSubmit: function (param) {
            param.cmd = 'submit_review_Report';
            param.ids = ids;
        ...
        success: function (data) {
            if (data == 'T') {
                $('#submit_dialog').dialog('close');
                if (level_ == 1) { $.messager.alert('提示', '提交签发成功！'); }
                else {

                    $.messager.alert('提示', '报告归档成功！');
                }
...
function management_all() {
    ...
    $('#group').combobox({
        url: "LosslessReport_Review.ashx?&cmd=load_professional_department",
        valueField: 'id',
